...
import datetime
import io
from typing import TYPE_CHECKING, Annotated, Any, Iterable, Optional, TypedDict
...
async def check_guild_permissions(ctx: DoomCtx, perms: dict[str, bool], *, check=all):
...
class TagPages(SimplePages):
    def __init__(self, entries: list[TagEntry], *, ctx: DoomCtx, per_page: int = 12):
...
class TagName(commands.clean_content):
    ...
    async def convert(self, ctx: DoomCtx, argument: str) -> str:
...
class TagEditModal(discord.ui.Modal, title="Edit Tag"):
    ...
    async def on_submit(self, interaction: DoomItx) -> None:
...
class TagMakeModal(discord.ui.Modal, title="Create New Tag"):
    ...
    def __init__(self, cog: Tags, ctx: DoomCtx):
        super().__init__()
        self.cog: Tags = cog
        self.ctx: DoomCtx = ctx
...
class Tags(commands.Cog):
    ...
    async def cog_command_error(self, ctx: DoomCtx, error: commands.CommandError):
    ...
    async def get_possible_tags(
        ...
        *,
        ...
        query = """SELECT name, content FROM tags2 WHERE location_id=$1;"""
    ...
    async def get_tag(
        ...
        *,
        ...
        query = """SELECT tags2.name, tags2.content
                   FROM tag_lookup
                   INNER JOIN tags2 ON tags2.id = tag_lookup.tag_id
    ...
    async def create_tag(self, ctx: DoomCtx, name: str, content: str) -> None:
    ...
    async def non_aliased_tag_autocomplete(self, interaction: DoomItx, current: str) -> list[app_commands.Choice[str]]:
        query = """SELECT name FROM tags2 WHERE location_id=$1 AND LOWER(name) % $2 LIMIT 12;"""
    ...
    async def aliased_tag_autocomplete(self, interaction: DoomItx, current: str) -> list[app_commands.Choice[str]]:
        query = """SELECT name FROM tag_lookup WHERE location_id=$1 AND LOWER(name) % $2 LIMIT 12;"""
        results: list[tuple[str]] = await self.bot.pool.fetch(query, interaction.guild_id, current.lower())
        return [app_commands.Choice(name=a, value=a) for a, in results]

    async def owned_non_aliased_tag_autocomplete(self, interaction: DoomItx, current: str) -> list[app_commands.Choice[str]]:
        query = """SELECT name
                   FROM tags2
    ...
    async def owned_aliased_tag_autocomplete(self, interaction: DoomItx, current: str) -> list[app_commands.Choice[str]]:
        query = """SELECT name
                   FROM tag_lookup
                   WHERE location_id=$1 AND owner_id=$2 AND name % $3
                   ORDER BY similarity(name, $3) DESC
    ...
    async def tag(self, ctx: DoomCtx, *, name: Annotated[str, TagName(lower=True)]):
        ...
        try:
            tag = await self.get_tag(ctx.guild.id, name, pool=ctx.bot.pool)
        ...
        # update the usage
        query = "UPDATE tags2 SET uses = uses + 1 WHERE name = $1 AND location_id=$2;"
    ...
    @commands.guild_only()
    async def make(self, ctx: DoomCtx):
        ...
        query = """SELECT 1 FROM tags2 WHERE location_id=$1 AND LOWER(name)=$2;"""
    ...
    @make.error
    async def tag_make_error(self, ctx: DoomCtx, error: commands.CommandError):
        ...
        *,
        ...
        if content is None:
            if ctx.interaction is None:
                ...
                query = "SELECT content FROM tags2 WHERE LOWER(name)=$1 AND location_id=$2 AND owner_id=$3"
                row: Optional[tuple[str]] = await self.bot.pool.fetchrow(query, name, ctx.guild.id, ctx.author.id)
                if row is None:
                    await ctx.send(
        ...
        query = "UPDATE tags2 SET content=$1 WHERE LOWER(name)=$2 AND location_id=$3 AND owner_id=$4;"
    ...
    async def remove(self, ctx: DoomCtx, *, name: Annotated[str, TagName(lower=True)]):
        ...
        query = f"DELETE FROM tags2 WHERE id=${len(args)} AND {clause};"
    ...
    async def remove_id(self, ctx: DoomCtx, tag_id: int):
        """Removes a tag by ID.
        ...
        query = f"DELETE FROM tags2 WHERE {clause};"
        status = await self.bot.pool.execute(query, *args)
    ...
    async def _send_alias_info(self, ctx: DoomCtx, record: asyncpg.Record):
    ...
    async def _send_tag_info(self, ctx: DoomCtx, record: asyncpg.Record):
        embed = discord.Embed(colour=discord.Colour.blurple())
    ...
    @app_commands.autocomplete(name=aliased_tag_autocomplete)
    async def info(self, ctx: DoomCtx, *, name: Annotated[str, TagName(lower=True)]):
        ...
        query = """SELECT
                       tag_lookup.name <> tags2.name AS "Alias",
    ...
    @staticmethod
    async def safe_send(context: DoomCtx, content: str, *, escape_mentions: bool = True, **kwargs) -> discord.Message:
        ...
        if len(content) > 2000:
            ...
            return await context.send(file=discord.File(fp, filename="message_too_long.txt"), **kwargs)
        else:
    ...
    async def raw(self, ctx: DoomCtx, *, name: Annotated[str, TagName(lower=True)]):
        ...
        await self.safe_send(ctx, first_step.replace("<", "\\<"), escape_mentions=False)

    @tag.command(name="list")
    @commands.guild_only()
    @app_commands.describe(member="The member to list tags of, if not given then it shows yours")
    async def _list(self, ctx: DoomCtx, *, member: discord.User = commands.Author):
    ...
    @app_commands.guild_only()
    @app_commands.describe(member="The member to list tags of, if not given then it shows yours")
    async def tags(self, ctx: DoomCtx, *, member: discord.User = commands.Author):
        """An alias for tag list command."""
        await ctx.invoke(self._list, member=member)

    async def _tag_all_text_mode(self, ctx: DoomCtx):
        ...
        bypass_owner_check = ctx.author.id == self.bot.owner_id or ctx.author.guild_permissions.manage_messages
        rows = await self.bot.pool.fetch(query, ctx.guild.id, bypass_owner_check, ctx.author.id)
        if not rows:
    ...
    @commands.guild_only()
    async def _all(self, ctx: DoomCtx, *, flags: TagAllFlags):
    ...
    async def purge(self, ctx: DoomCtx, member: discord.User):
        ...
        query = "SELECT COUNT(*) FROM tags2 WHERE location_id=$1 AND owner_id=$2;"
        row: tuple[int] = await self.bot.pool.fetchrow(query, ctx.guild.id, member.id)  # type: ignore
        ...
        query = "DELETE FROM tags2 WHERE location_id=$1 AND owner_id=$2;"
    ...
    async def search(self, ctx: DoomCtx, *, query: Annotated[str, commands.clean_content]):
        """Searches for a tag.
    ...
    async def claim(self, ctx: DoomCtx, *, tag: Annotated[str, TagName]):
        ...
        query = "SELECT id, owner_id FROM tags2 WHERE location_id=$1 AND LOWER(name)=$2;"
        ...
        if row is None:
            ...
            row = await self.bot.pool.fetchrow(alias_query, ctx.guild.id, tag.lower())
            if row is None:
        ...
        async with self.bot.pool.acquire() as conn:
            async with conn.transaction():
                if not alias:
                    query = "UPDATE tags2 SET owner_id=$1 WHERE id=$2;"
    ...
    @app_commands.autocomplete(tag=aliased_tag_autocomplete)
    async def transfer(self, ctx: DoomCtx, member: discord.Member, *, tag: Annotated[str, TagName]):
        ...
        query = "SELECT id FROM tags2 WHERE location_id=$1 AND LOWER(name)=$2 AND owner_id=$3;"

        row = await self.bot.pool.fetchrow(query, ctx.guild.id, tag.lower(), ctx.author.id)
        if row is None:
    ...
    async def config(self, ctx: DoomCtx):
        """This is a reserved tag command. Check back later."""
        pass

    @tag.command()
    async def random(self, ctx: DoomCtx):
